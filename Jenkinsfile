        ...
        stage('Deploy to Staging') {
            steps {
                ...
                sshagent(['vue-nginx-1']) {
                    ...
                    sh "ssh -o StrictHostKeyChecking=no -r ${STAGING_SERVER} 'ls'"
                    sh "ssh -o StrictHostKeyChecking=no -r ${STAGING_SERVER} 'cd src && npm install --no-audit --no-fund && npm run start'"
                }
            }
        }
